...
def build_dji_kmz(destination: Path, route, altitude_m, speed_mps, gimbal_pitch, photo_mode, photo_distance_m, route_mode, finish_action, signal_loss_action):
    ...
    for i,(lat,lon) in enumerate(route):
        ...
        marks.append(f"<Placemark><Point><coordinates>{lon:.8f},{lat:.8f}</coordinates></Point><wpml:index>{i}</wpml:index><wpml:executeHeight>{altitude_m:.1f}</wpml:executeHeight><wpml:waypointSpeed>{speed_mps:.1f}</wpml:waypointSpeed><wpml:waypointTurnParam><wpml:waypointTurnMode>{turn}</wpml:waypointTurnMode><wpml:waypointTurnDampingDist>0</wpml:waypointTurnDampingDist></wpml:waypointTurnParam><wpml:gimbalPitchAngle>{gimbal_pitch:.1f}</wpml:gimbalPitchAngle>{action}</Placemark>")
    interval=""
