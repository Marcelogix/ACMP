...
def build_dji_kmz(destination: Path, route, altitude_m, speed_mps, gimbal_pitch, photo_mode, photo_distance_m, route_mode, finish_action, signal_loss_action, waypoint_speeds=None, use_point_gimbal_pitch=True, waypoint_altitudes=None, waypoint_gimbal_pitches=None):
    ...
    for i,(lat,lon) in enumerate(route):
        ...
        gimbal_action = ""
        point_gimbal_pitch = waypoint_gimbal_pitches[i] if waypoint_gimbal_pitches is not None else gimbal_pitch
        if use_point_gimbal_pitch and (waypoint_gimbal_pitches is not None or i == 0):
            gimbal_action = (
                f"<wpml:actionGroup><wpml:actionGroupId>{10000+i}</wpml:actionGroupId>"
                f"<wpml:actionGroupStartIndex>{i}</wpml:actionGroupStartIndex><wpml:actionGroupEndIndex>{i}</wpml:actionGroupEndIndex>"
                ...
                f"<wpml:gimbalPitchRotateAngle>{point_gimbal_pitch:.1f}</wpml:gimbalPitchRotateAngle>"
